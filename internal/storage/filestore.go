...
func (s *FileStore) Put(id string, version uint64, text string) {
	_ = os.MkdirAll(s.reqDir(id), 0o755)
	_ = os.WriteFile(s.verPath(id, version), []byte(text), 0o644)
	_ = os.WriteFile(s.latestPath(id), []byte(fmt.Sprintf("%d", version)), 0o644)
}
